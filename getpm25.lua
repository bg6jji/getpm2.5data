    ...
    function ledflash(flashtime)
      gpio.write(5,gpio.HIGH) tmr.delay(flashtime*1000) gpio.write(5,gpio.LOW) tmr.delay(flashtime*1000)
      gpio.write(5,gpio.HIGH) tmr.delay(flashtime*1000) gpio.write(5,gpio.LOW) tmr.delay(flashtime*1000)
    end
    ...
    tmr.alarm(0, 30000, 1, function()   --请将抓取时间改大一些，减少服务器负载。
     ...
     local tempdata=string.find(response,"aqi\"")
      ...
      if tempdata then
          ...
          print("监测点:"..pn.."  采样时间:"..time1)
          print(node.heap())
          aqidata = tonumber(aqidata) pm10 = tonumber(pm10) pm25 = tonumber(pm25)
          led(aqidata, pm10, pm25)    --LED按数值点亮，越亮污染越重
          if aqidata >= 200 then          ledflash(1500)
            elseif aqidata >= 150 then    ledflash(600) 
               elseif aqidata >= 100 then ledflash(200) end
          
          
      end
      socket:close()
      end)
      --socket:on("disconnection", function(conn, pl) print("disconnection from:"..Server) socket:close() node.restart() end) 
   end)
